...
impl Scope {
    pub fn new(task: String, files: Option<String>, exclude: Option<String>, project: Option<PathBuf>) -> Result<Self> {
    ...
    pub fn files_pattern(&self) -> Option<Vec<&str>> {
        self.files.as_ref().map(|f| f.split(',').map(|s| s.trim()).collect())
    }

    pub fn exclude_pattern(&self) -> Option<Vec<&str>> {
        self.exclude.as_ref().map(|e| e.split(',').map(|s| s.trim()).collect())
    }

    pub fn is_file_in_scope(&self, file_path: &str) -> bool {
        let file_path_clean = file_path.replace('\\', "/").trim_start_matches("./").to_string();
        ...
        if let Some(ref exclude_patterns) = self.exclude {
            for pat in exclude_patterns.split(',') {
                ...
                if let Ok(pattern) = Pattern::new(pat) {
                    if pattern.matches(&file_path_clean) || pattern.matches(&format!("/{}", &file_path_clean)) {
                        return false;
                    }
                }
            }
        ...
        let in_files = if let Some(ref patterns) = self.files {
            patterns.split(',').any(|pat| {
                let pat = pat.trim();
                if let Ok(pattern) = Pattern::new(pat) {
                    pattern.matches(&file_path_clean) || pattern.matches(&format!("/{}", file_path_clean))
                } else {
                    false
                }
            })
    ...
    pub fn is_blocked_file(&self, file_path: &str) -> bool {
        ...
        for pat in BLOCKED_PATTERNS {
            if let Ok(pattern) = glob::Pattern::new(pat) {
                ...
                if !basename.is_empty() && (pattern.matches(basename) || pattern.matches(&format!("/{}", basename))) {
                    return true;
                }
